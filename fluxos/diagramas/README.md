...
docs/
  fluxogramas.md             Todos os fluxogramas em um arquivo
  fluxogramas/{id}.md        Um fluxograma por fluxo
scripts/
  validar_fluxos.py          Validador de estrutura e de integridade do grafo
  gerar_diagramas.py         Gerador dos fluxogramas em Mermaid
```
...
Os diagramas são **gerados a partir do JSON**, nunca desenhados à mão. Alterou um
fluxo, roda o script e a documentação acompanha — não existe o risco de o desenho
divergir do que o sistema realmente faz.

O formato é Mermaid, que o GitHub renderiza nativamente no navegador. Cada diagrama
traz as perguntas em hexágonos azuis e as orientações finais em retângulos coloridos
conforme o encaminhamento (vermelho para agendamento presencial, amarelo para
informativo, verde para PROCON online, cinza para fora de competência).

Os nós terminais aparecem com rótulo curto para o diagrama continuar legível; o texto
de cada orientação está na tabela logo abaixo de cada fluxograma.

Para o QA: cada caminho da raiz até um retângulo é uma conversa completa possível e
serve diretamente como caso de teste. São 46 caminhos no total.
